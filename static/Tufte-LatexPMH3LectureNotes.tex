...
\setcounter{secnumdepth}{1} % turn on numbering for parts and chapters 
...
\begin{prop}
  $(\ell^p, \| \cdot \|_p)$ is a Banach space for all $1 \leq p \leq \infty$.\sidenote{
...
\begin{thm}[Completion] Let $(V, \| \cdot \| )$ be a normed vector space over $\K$.  There exists a Banach space ($V_1, \| \cdot \|_1$) such that $( V, \| \cdot \|)$ is isometrically isomorphic to a dense subspace of $(V_1, \| \cdot \|_1)$.  
  ...
  Furthermore, the space $( V_1, \| \cdot \|_1)$ is unique up to isometric isomorphisms.\sidenote{The proof of this fact is rather straightforward;
  \begin{enumerate}[(i)]
    \item construct Cauchy sequences,
    \item append limits,
    \item quotient out (as different sequences may converge to the same limit)
  \end{enumerate}}
\end{thm}
...
\begin{thm}[Riesz-Fischer] $(L^p([a,b]), \| \cdot \|_p )$ is the completion of $(\mathcal{C}[a,b],  \| \cdot \|_p )$, and is a Banach space.\sidenote{This fact follows quite easily from the definition of the Lebesgue integral.}
\end{thm}
...
% section projection_operators (end)
...
\begin{exmp}
  ...
  \] for some $g \in L^2([a,b])$.\sidenote{As an exercise, show that this is true.}
\end{exmp}
...
\begin{exmp}[Adjoint operators]
  ...
  \] for all $x \in \H_1, y \in \H_2$\sidenote{As an exercise, show that for a matrix $T$, we have $T^\star = \overline{T^t}$ where $T^t$ is the matrix transpose.}
\end{exmp}
% 
% \begin{exer}
...
% section orthonormal_systems (end)
...
\begin{defn}[Complete orthonormal system or Hilbert basis]
  An orthonormal system $S$ is \textbf{complete} or a \textbf{Hilbert basis}  \sidenote{
...
\begin{proof}
  If $\sum_{k=0}^\infty a_k e_k$ converges to $x$, then\sidenote{
...
% section the_stone_weierstrass_theorem (end)
% \subsection{Stone-Weierstrass theorem} % (fold)
% \label{sub:stone_weierstrass_theorem}
\newthought{This is a useful tool} to show an orthonormal system is complete.  In fact, this theorem is about uniformly approximating elements of $\mathcal{C}(X)$, where $X$ is a compact Hausdorff space.  it is a generalisation of the Weierstrass approximation theorem.
...
\begin{proof}
  ...
  \sidenote{As an exercise, show that if $X$ is a compact metric space, then $X$ is separable.}
...
\newthought{The following sections} introduce two of the cornerstone theorems of functional analysis - the uniform boundedness principle and the open mapping theorem.
...
\begin{thm}[Uniform boundedness]
  ...
  \] for each fixed $x \in X$, then\sidenote{
              This is an amazing result --- we obtain a global bound from pointwise bounds.} \[
...
\] where $e_k(t) = \frac{e^{ikt}}{\sqrt{2 \pi}}$.  This converges to $f$ in the $L^2$ norm.\sidenote{It is useful here to attempt to prove or disprove this statement.\begin{exer}
...
\newthought{This theorem is tailor-made} to deal with inverse operators.
...
\begin{thm}[Closed Graph theorem]
  Let $X, Y$ be Banach spaces, and $T \in \text{Hom}(X,Y)$.  Then $T \in \mathcal L(X,Y)$ if and only if $\Gamma(T)$ is closed in $X \times Y$.\sidenote{Prove the following statement.
    \begin{exer}
...
\newthought{Recall that the} eigenvalues of an $n \times n$ matrix $T$ over $\C$ are the $\lambda \in \C$ with \[
  \det(\lambda I - T) = 0
\] that is, $\lambda I - T$ is not invertible.\sidenote{Recall that showing the existence of eigenvalues is equivalent to the fundamental theorem of algebra.}\sidenote{We need our base field to be $\C$ to get reasonable spectral theory.}
...
\begin{defn}
  Let $X$ be a Banach space over $\K$, and let $T \in \mathcal L(X)$.  Then the spectrum of $T$ is\sidenote{$\lambda I - T$ is non invertible if either $\lambda I - T$ is not injective, or $\lambda I - T$ is not surjective.}
   \[
...
$\lambda \in \K$ is an eigenvalue of $T \in \mathcal L(X)$ if there is $ x \neq 0$ with $Tx = \lambda x$, i.e. $\lambda$ is an eigenvalue if and only if $\lambda I - T$ is not injective.\sidenote{
...
\begin{exmp}
  ...
  Then $\| R \| = 1$, and so $\sigma(R) \subseteq \overline D(0,1)$.\sidenote{
      $LR(x) = L(0, x_1, \dots) = (x_1, x_2, \dots)$, so \[
          LR = I \tag{$\star$}
      \]
      \begin{rem}
...
\newthought{We now turn to} compact operators.  In general, calculating $\sigma(T)$ is difficult, but for compact operators on a complex Banach space, we have a fairly explicit theory.
...
\begin{cor}
  The set $\{ \text{compact operators $T : X \rightarrow Y$}\}$ is a vector space.  That is, if $T_1, T_2$ are compact, then $T_1 + T_2$ and $\alpha T_1$ are compact.\sidenote{As an exercise, prove this statement. Use $(c)$ from Theorem \ref{thm:charcompact}.}
\end{cor}
...
\newthought{We are now ready} to prove the following theorem.
...
\newthought{Recall that we had} the following theorem, a corollary in the previous chapter.
\begin{thm}
  Let $X$ be Banach, $T \in \mathcal K(X)$, $\lambda \neq 0$.  Let $r = \alpha(\lambda I - T) = \delta(\lambda I - T)$.  Then $X = \ker(\lambda I - T)^r \oplus \im(\lambda I - T)^r$ and this completely reduces $\mu I - T, \mu \in \K$.\sidenote{Indeed, consider the following exercise, which shows us how we can diagonalise an arbitrary operator.
  \begin{exer}
    Let $\lambda_1, \dots, \lambda_n \in \sigma(T) \backslash \{ 0 \}$.  Let $N_j = \ker(\lambda_j I - T)^{r_j}$ be the generalised $\lambda_j$-eigenspace.  Show that there exists closed subspaces $M$ with \[
...
$T \in \mathcal L(\mathcal H)$ is\sidenote{
      Recall that for a matrix $A$, we can restate this as follows, \begin{enumerate}[(a)]
...
\newthought{The spectral theory for} compact normal operators in a Hilbert space is particularly nice, as the following theorem demonstrates.
\begin{thm}
...
\begin{proof}
  ...
  \] is compact and normal.\sidenote{As an exercise, prove this statement.} Then  \begin{enumerate}[(a)]
...
\newthought{Recall that for matrices}, we have the following result, known as the Fredholm alternative.    
